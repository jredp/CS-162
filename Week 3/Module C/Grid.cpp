...
/*------------------------------------------------------------------------------*/
Ant*** Grid::createGridArray(int xRows, int yCols) {
	//New Grid of pointers to Critters
	Ant*** tempGrid;
	tempGrid = new Ant**[xRows];
	for (int i = 0; i < xRows; ++i) {
		tempGrid[i] = new Ant*[yCols];
	}
	...
	for (int i = 0; i < desiredCritters; i++) {
		...
		while (critterPlaced == false) {
			...
			if (gridOfCritters[randX][randY] == NULL) {
				gridOfCritters[randX][randY] = new Ant(); // Place new Critter adjust for Array
				critterPlaced = true;
			}
		}
	}
