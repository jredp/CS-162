...
void Grid::clearStarved(int x, int y) {
	//Kill this Critter slot
	gridOfCritters[x][y] = NULL;
	//std::cout << "Starved! ";
}
/*------------------------------------------------------------------------------*/
...
//	Critter* curCrit = gridOfCritters[currentX][currentY];
//	char curIcon = curCrit->getIcon(); //Get current Critter Icon
//	
//	if (direction == 1) {
//		proposedX--; //move 1 up = x-1 / proposedX
//		if (proposedX >= 0) {
//			//If slot not NULL
//			if (gridOfCritters[proposedX][currentY] != NULL) {			
//				//If slot is a Doodlebug
//				if (curIcon == '#') {
//					char propIcon = gridOfCritters[proposedX][currentY]->getIcon();
//					//If Next Slot is Ant
//					if (propIcon == '*') {
//						//Doodlebug* pointDbug;
//						//pointDbug = static_cast<Doodlebug *>(curCrit); //Derived class pointer set
//						gridOfCritters[proposedX][currentY] = gridOfCritters[currentX][currentY]; //Copy Pointer over this slot (eat)
//						gridOfCritters[currentX][currentY] = gridOfCritters[proposedX][currentY]->Breed(); //Breed current square
//						gridOfCritters[proposedX][currentY]->setHasMoved(true); //Set Critter hasMoved to true
//						gridOfCritters[proposedX][currentY]->Eat(); //Mark eaten, reset moves
//					}
//					//ELSE DO NOTHING: Dont move, increase moves made, increase movesMadeNoEat					
//				}
//			}		
//			//Else if slot empty
//			else {
//				if (gridOfCritters[proposedX][currentY] == NULL) {
//					gridOfCritters[proposedX][currentY] = gridOfCritters[currentX][currentY]; //Copy Pointer to this slot	
//					gridOfCritters[currentX][currentY] = gridOfCritters[proposedX][currentY]->Breed(); //Breed current square
//					gridOfCritters[proposedX][currentY]->setHasMoved(true); //Set Critter hasMoved to true			
//				}
//			}
...
//			gridOfCritters[currentX][currentY] = gridOfCritters[currentX][proposedY]->Breed(); //Breed current square
